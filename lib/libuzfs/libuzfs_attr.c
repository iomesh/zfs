						 ...
						 uint64_t size, int flags, uint64_t *txg)
{
    ...
    if (err_check_sa != 0 && err_check_dir == 0) {
        ...
        zap_count(os, xattr_obj, &dir_entries);
    ...
    if (!((err_check_sa == 0 || err_check_dir != 0) && err_check_sa_enough == 0)) {
    ...
    if ((err_check_sa_enough == 0 || value == NULL) &&
        ...
        if (dir_entries == 1) {
            VERIFY0(zap_destroy(os, xattr_obj, tx));
            VERIFY0(sa_remove(sa_hdl, attr_tbl[UZFS_DXATTR], tx));
        } else {
